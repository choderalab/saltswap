...
f.write("Total work to remove salt for each attempt\n")
s = "Niterations = {:4}, Nsteps = {:7}, Nattemps = {:3}, Dchem = {:5}, Nkernals = {:5}, Nverlet = {:3}\n".format(args.cycles,args.steps,args.attempts,args.deltachem,args.nkernals,args.nverlet)
f.write(s)
f.close()
...
f.write("Total work to add salt for each attempt\n")
s = "Niterations = {:4}, Nsteps = {:7}, Nattemps = {:3}, Dchem = {:5}, Nkernals = {:5}, Nverlet = {:3}\n".format(args.cycles,args.steps,args.attempts,args.deltachem,args.nkernals,args.nverlet)
f.write(s)
f.close()
...
for i in range(iterations):
    ...
    if len(mc_saltswap.work_add) > 0:
        f = open("work_add_"+args.data,"a")
        f.writelines("%s " % item  for item in mc_saltswap.work_add)
        f.write("\n")
        f.close()
    if len(mc_saltswap.work_rm) > 0:
        f = open("work_rm_"+args.data,"a")
        f.writelines("%s " % item  for item in mc_saltswap.work_rm)
        f.write("\n")
        f.close()
    mc_saltswap.work_add=[]
    mc_saltswap.work_rm=[]
    # Saving structures
    positions = context.getState(getPositions=True,enforcePeriodicBox=True).getPositions(asNumpy=True)
